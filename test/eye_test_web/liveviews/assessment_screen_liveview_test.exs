defmodule EyeTestWeb.AssessmentScreenLiveviewTest do
  ...
  test "full normal workflow of an assessment", %{conn: conn} do
    ...
    message_and_response(lv, "start_test", "test-assessment-question-countdown")
    message_and_response(lv, "reveal_question", "test-assessment-question-reveal")

    # Correct button pressed (1 correct)
    message_and_response(lv, {"button_pressed", get_displayed_letter(lv)}, "result-correct")
    message_and_response(lv, "next_question", "question-countdown")
    message_and_response(lv, "reveal_question", "question-reveal")
